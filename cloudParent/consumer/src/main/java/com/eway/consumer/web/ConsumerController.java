...
//    }) 
    @HystrixCommand(commandProperties = {
        @HystrixProperty(name = "circuitBreaker.requestVolumeThreshold",value = "10"),//触发断路器的最小请求量，即请求10次统计一次，默认是20
        @HystrixProperty(name = "circuitBreaker.sleepWindowInMilliseconds",value = "10000"),//断路器打开休眠时间窗是10秒 ，默认是5秒
        @HystrixProperty(name = "circuitBreaker.errorThresholdPercentage", value="60")//请求10次中超过60%的服务是不正常的就打开熔断器 ，默认是50%
    })
    //一旦触发就会找默认的defaultFallback里的方法，这里已经在配置文件中配置了超时的时间了，所以不用加注解了
    public String queryById(@PathVariable("id") Long id) {
        //手动实现一个控制熔断器的小技巧 测试用的
        if(id % 2 ==1) {
